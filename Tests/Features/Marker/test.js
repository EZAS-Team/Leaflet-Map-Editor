...
function FeatureMakerCreationTest()
{
    let Markers = []
    let ids = []
    for(let i=0; i < 10; i++)
    ...
    while(Markers.length()>0)
    {
        ...
        (ids.find(this == tempexport.id))?results.failed++:results.passed++;
        //test that the latlng is [0,0]
        (tempexport.parameters.latlng == [0,0])?results.passed++:results.failed++;
        //test that the options are empty
        (tempexport.parameters.options == {})?results.passed++:results.failed++;
        ...
        (tempexport.object == new L.marker(parameters.latlng,parameters.options))?results.passed++:results.failed++;
